...
describe("UserController", () => {
    ...
    beforeEach(() => {
        userController = new UserController(mockApp, mockUserService, mockAccessTokenService);
    ...
    describe("authUserApi", () => {
        it("should authenticate a user and return a success message", async () => {
            const mockUser = { id: 1, email: "test@test.com", password_hash: "hashedPassword" };
            ...
            mockAccessTokenService.createAccessToken = jest.fn().mockResolvedValue(mockToken);
            ...
            expect(mockUserService.getUser).toHaveBeenCalledWith({ where: { email: "test@test.com" } });
        });
