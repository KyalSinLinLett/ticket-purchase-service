...
describe('TicketService unit tests', () => {
    ...
    it('should get a ticket by conditions', async () => {
        const condition: FindOptions<TicketDetails> = { where: { event_id: 'event1' } };
    ...
    it('should handle create ticket failure', async () => {
        ...
        await expect(ticketService.createTicket(mockTicketData)).rejects.toThrow('Creation failed');
    });

    it('should handle find ticket failure', async () => {
        const condition: FindOptions<TicketDetails> = { where: { event_id: 'nonexistent' } };
        ticketRepositoryMock.findOne.mockRejectedValue(new Error('Find failed'));

        await expect(ticketService.getTicket(condition)).rejects.toThrow('Find failed');
    });
    ...
    it('should handle get all tickets failure', async () => {
        ...
        await expect(ticketService.getAllTickets({})).rejects.toThrow('Get all failed');
    });
    ...
    it('should handle update ticket failure', async () => {
        ...
        await expect(ticketService.updateTicket(id, updateData as any)).rejects.toThrow('Update failed');
    });
    ...
    it('should handle delete ticket failure', async () => {
        ...
        await expect(ticketService.deleteTicket(id)).rejects.toThrow('Delete failed');
    });
    ...
    it('should create a ticket with minimal data', async () => {
        ...
        const result = await ticketService.createTicket(minimalTicketData as TicketDetails);
    ...
    it('should handle create ticket with extra fields', async () => {
        ...
        const result = await ticketService.createTicket(extraFieldsData as TicketDetails);
    ...
    it('should handle ticket creation with incomplete data', async () => {
        ...
        ticketRepositoryMock.create.mockRejectedValue(new Error('Invalid ticket data'));

        await expect(ticketService.createTicket(incompleteData as TicketDetails)).rejects.toThrow('Invalid ticket data');
    });
    ...
    it('should handle finding tickets with multiple filters', async () => {
        const filter: FindOptions<TicketDetails> = { where: { price: { [Op.gt]: 50 } } };
