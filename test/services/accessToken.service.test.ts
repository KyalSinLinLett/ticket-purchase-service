...
describe('AccessTokenService unit tests', () => {
  ...
  it('should create an access token', async () => {
    accessTokenRepositoryMock.create.mockResolvedValue(
      mockAccessTokenData as any,
    )
    ...
    expect(accessTokenRepositoryMock.create).toHaveBeenCalledWith(
      mockAccessTokenData,
    )
  ...
  it('should get an access token by condition', async () => {
    const condition: FindOptions<AccessToken> = {
      where: { token: 'sample-token' },
    }
    accessTokenRepositoryMock.findOne.mockResolvedValue(
      mockAccessTokenData as any,
    )
  ...
  it('should get an access token by ID', async () => {
    accessTokenRepositoryMock.findById.mockResolvedValue(
      mockAccessTokenData as any,
    )

    const result = await accessTokenService.getAccessTokenById(
      mockAccessTokenData.id,
    )

    expect(accessTokenRepositoryMock.findById).toHaveBeenCalledWith(
      mockAccessTokenData.id,
    )
    expect(result).toEqual(mockAccessTokenData)
  })
  ...
  it('should get all access tokens', async () => {
    accessTokenRepositoryMock.findAll.mockResolvedValue([
      mockAccessTokenData,
    ] as any)
  ...
  it('should update an access token', async () => {
    ...
    accessTokenRepositoryMock.update.mockResolvedValue(
      updatedAccessTokenData as any,
    )

    const result = await accessTokenService.updateAccessToken(
      mockAccessTokenData.id,
      updatedAccessTokenData,
    )

    expect(accessTokenRepositoryMock.update).toHaveBeenCalledWith(
      mockAccessTokenData.id,
      updatedAccessTokenData,
    )
  ...
  it('should delete an access token', async () => {
    ...
    const result = await accessTokenService.deleteAccessToken(
      mockAccessTokenData.id,
    )

    expect(accessTokenRepositoryMock.delete).toHaveBeenCalledWith(
      mockAccessTokenData.id,
    )
  ...
  it('should handle create access token failure', async () => {
    accessTokenRepositoryMock.create.mockRejectedValue(
      new Error('Creation failed'),
    )

    await expect(
      accessTokenService.createAccessToken(mockAccessTokenData),
  ...
  it('should handle get access token failure', async () => {
    ...
    accessTokenRepositoryMock.findOne.mockRejectedValue(
      new Error('Find failed'),
    )

    await expect(accessTokenService.getAccessToken(condition)).rejects.toThrow(
      'Find failed',
    )
  })

  it('should handle get access token by ID failure', async () => {
    accessTokenRepositoryMock.findById.mockRejectedValue(
      new Error('Find by ID failed'),
    )

    await expect(
      accessTokenService.getAccessTokenById(mockAccessTokenData.id),
    ).rejects.toThrow('Find by ID failed')
  })

  it('should handle get all access tokens failure', async () => {
    accessTokenRepositoryMock.findAll.mockRejectedValue(
      new Error('Get all failed'),
    )

    await expect(accessTokenService.getAllAccessTokens()).rejects.toThrow(
      'Get all failed',
    )
  })
  ...
  it('should handle update access token failure', async () => {
    ...
    accessTokenRepositoryMock.update.mockRejectedValue(
  ...
  it('should handle delete access token failure', async () => {
    ...
    accessTokenRepositoryMock.delete.mockRejectedValue(
      new Error('Delete failed'),
    )

    await expect(
      accessTokenService.deleteAccessToken(mockAccessTokenData.id),
