...
describe('EventService unit tests', () => {
  ...
  it('should handle update event failure', async () => {
    ...
    await expect(
      eventService.updateEvent(mockEventData.id, updatedEventData),
  ...
  it('should handle delete event failure', async () => {
    ...
    await expect(eventService.deleteEvent(mockEventData.id)).rejects.toThrow(
      'Delete failed',
    )
  })
  ...
  it('should return empty array when no events are found', async () => {
    ...
    expect(eventRepositoryMock.findAll).toHaveBeenCalledWith({
      where: {},
      include: { model: TicketCategory, as: 'ticketCategories' },
    })
  ...
  it('should handle event creation with missing required fields', async () => {
    ...
    eventRepositoryMock.create.mockRejectedValue(
      new Error('Invalid event data'),
    )

    await expect(eventService.createEvent(incompleteEventData)).rejects.toThrow(
      'Invalid event data',
    )
  })
  ...
  it('should return null when no event is found with the given condition', async () => {
    const condition: FindOptions<Event> = {
      where: { name: { [Op.like]: `%nonexistent_event%` } },
      include: { model: TicketCategory, as: 'ticketCategories' },
    }
    eventRepositoryMock.findAll.mockResolvedValue([])
  ...
  it('should correctly handle missing optional fields', async () => {
    ...
    eventRepositoryMock.create.mockResolvedValue(
      eventDataWithMissingOptionalFields as any,
    )

    const result = await eventService.createEvent(
      eventDataWithMissingOptionalFields,
    )
  ...
  it('should handle missing required fields during update', async () => {
    ...
    eventRepositoryMock.findById.mockResolvedValue(mockEventData as any)
    eventRepositoryMock.update.mockRejectedValue(
      new Error('Invalid update data'),
    )

    await expect(
      eventService.updateEvent(mockEventData.id, incompleteUpdateData),
