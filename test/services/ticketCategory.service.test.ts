...
describe('TicketCategoryService unit tests', () => {
  ...
  it('should create a ticket category', async () => {
    ticketCategoryRepositoryMock.create.mockResolvedValue(
      mockTicketCategoryData as any,
    )

    const result = await ticketCategoryService.createTicketCategory(
      mockTicketCategoryData,
    )

    expect(ticketCategoryRepositoryMock.create).toHaveBeenCalledWith(
      mockTicketCategoryData,
    )
  ...
  it('should create multiple ticket categories', async () => {
    ...
    const result = await ticketCategoryService.createBulkTicketCategory(
      categories,
    )

    expect(ticketCategoryRepositoryMock.createBulk).toHaveBeenCalledWith(
      categories,
    )
  ...
  it('should update a ticket category', async () => {
    ...
    ticketCategoryRepositoryMock.update.mockResolvedValue(
      updatedCategory as any,
    )

    const result = await ticketCategoryService.updateTicketCategory(
      id,
      updateData,
    )

    expect(ticketCategoryRepositoryMock.update).toHaveBeenCalledWith(
      id,
      updateData,
    )
  ...
  it('should delete a ticket category', async () => {
    ...
    ticketCategoryRepositoryMock.delete.mockResolvedValue(
      mockTicketCategoryData as any,
    )
  ...
  it('should handle create ticket category failure', async () => {
    ticketCategoryRepositoryMock.create.mockRejectedValue(
      new Error('Creation failed'),
    )

    await expect(
      ticketCategoryService.createTicketCategory(mockTicketCategoryData),
    ).rejects.toThrow('Creation failed')
  })

  it('should handle create multiple ticket categories failure', async () => {
    const categories: TicketCategoryDetails[] = [mockTicketCategoryData]
    ticketCategoryRepositoryMock.createBulk.mockRejectedValue(
      new Error('Bulk creation failed'),
    )

    await expect(
      ticketCategoryService.createBulkTicketCategory(categories),
  ...
  it('should handle get all ticket categories failure', async () => {
    ticketCategoryRepositoryMock.findAll.mockRejectedValue(
      new Error('Get all failed'),
    )

    await expect(
      ticketCategoryService.getAllTicketCategories(),
  ...
  it('should handle update ticket category failure', async () => {
    ...
    ticketCategoryRepositoryMock.update.mockRejectedValue(
      new Error('Update failed'),
    )

    await expect(
      ticketCategoryService.updateTicketCategory(id, updateData),
  ...
  it('should handle delete ticket category failure', async () => {
    ...
    ticketCategoryRepositoryMock.delete.mockRejectedValue(
  ...
  it('should handle creation of ticket category with missing optional fields', async () => {
    ...
    ticketCategoryRepositoryMock.create.mockResolvedValue(
      partialTicketCategoryData as any,
    )
