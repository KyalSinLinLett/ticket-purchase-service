...
export class App {
  routeMap: Record<
    string,
    { handler: RequestHandler; validator?: ValidationChain[] }
  > = {}
  apiPrefix = ''
  version = ''
  serviceName = ''
    ...
    for (const routePath in this.routeMap) {
      if (NO_AUTH_PATHS.includes(routePath)) {
        app.use(
          ...
          this.validate,
          this.routeMap[routePath].handler,
        )
      } else {
        app.use(
          routePath,
  ...
  validate: RequestHandler = (
    ...
    if (!errors.isEmpty()) {
      return res
        ...
        .json({ status: 0, error: errors.array() })
    }
    next()
    return
  }
  ...
  authMiddleware: RequestHandler = async (req, res, next): Promise<void> => {
    ...
    try {
      const decToken = decrypt(
        token as string,
        Buffer.from(configuration.encryptionKey, 'base64'),
      )
      ...
      const acsTkn = await accessTokenService.getAccessToken({
        where: { token: token as string, user_id: user.id, invalidated: false },
      })
      if (!acsTkn) {
        res
          .status(HttpStatusCode.UNAUTHORIZED)
