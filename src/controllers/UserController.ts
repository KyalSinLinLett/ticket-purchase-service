...
export class UserController {
  ...
  newUserApi: RequestHandler = async (
    req,
    res,
    next,
  ...
  authUserApi: RequestHandler = async (
    req,
    res,
    next,
  ): Promise<JsonResponse> => {
    try {
      ...
      const token = await this.accessTokenService.getAccessToken({
        where: { user_id: user.id, invalidated: false },
      })

      if (!!token)
        await this.accessTokenService.updateAccessToken(token.token, {
          invalidated: true,
        } as InferAttributes<AccessToken>)
